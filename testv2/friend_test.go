...
func Test_GetRecvFriendApplicationList(t *testing.T) {
	infos, err := open_im_sdk.UserForSDK.Friend().GetFriendApplicationListAsRecipient(ctx, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func Test_GetSendFriendApplicationList(t *testing.T) {
	infos, err := open_im_sdk.UserForSDK.Friend().GetFriendApplicationListAsApplicant(ctx, 0)
	if err != nil {
		t.Fatal(err)
	}
	for _, info := range infos {
